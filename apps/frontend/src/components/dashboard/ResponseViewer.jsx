...
function ResponseViewer({ formId, responseId }) {
  const { 
    response, 
    isLoading, 
    sendEmail, 
  ...
  const handlePlayAudio = (answerId, audioUrl) => {
    playAudio(answerId, audioUrl);
  };

  const getProgressWidth = (audioProgress) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          ...
          <Card>
            ...
            <Card.Content>
              <div className="space-y-4">
                {response.answers && response.answers.filter(a => a.audioUrl).map((answer, index) => (
                ...
                {(!response.answers || !response.answers.some(a => a.audioUrl)) && (
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {(response.keyTopics || ["Customer Service", "Mobile App", "Performance"]).map((topic, index) => (
